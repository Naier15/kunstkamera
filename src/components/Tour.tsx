...
import './styles.scss'
import { showModal, setHistory, Modal, ModalInfoType, ExampleDataType } from '../store/modalSlice'
...
const Tour = () => {
	...
	const [modalData, setModalData] = useState<Modal>({} as Modal)
	const [modalHistory, setModalHistory] = useState<ModalHistory>(
		{} as ModalHistory
	)
	const canvas = useRef<HTMLCanvasElement>(null)
	const gen = useRef<(arg: number) => void>()
	...
	const createScene = () => {
		...
		mainScene.start()
		mainScene.loop()
		return (stage: number) => mainScene?.onRoomChanged(stage)
	}

	const handlers = async (ev: KeyboardEvent) => {
		if (ev.code === 'KeyL') {
			await MainScene.instance.toggleLinks()
		}
		if (ev.ctrlKey && ev.code === 'KeyS') {
			ev.preventDefault()
			await MainScene.instance.saveCache()
		}
		if (ev.code === 'Tab') {
			ev.preventDefault()
			setEditMode(prev => {
				const newValue = (prev + 1) % 3
				MainScene.toggleChangeMode(newValue as EditMode)
				return newValue
			})
		}
	}
	...
	useEffect(() => {
		if (location.pathname === '/admin') {
			authorize(setAuthorized)
			document.addEventListener('keydown', handlers)
			return () => document.removeEventListener('keydown', handlers)
		}
	}, [])

	useEffect(() => {
		if (Object.keys(modalData).length === 0) return
		dispatch(showModal(modalData))
		dispatch(setHistory(modalHistory))
	}, [modalData])

	useEffect(() => {
		if (location.pathname === '/admin' && !authorized) return
		if (!gen.current) {
			gen.current = createScene()
		}
		const params = new URLSearchParams(location.search)
		const stageNum = params.has('stage') ? parseInt(params.get('stage')!) : 1
		if (stage === stageNum) return
		setStage(stageNum)
		gen.current?.(stageNum)
		return () => {
			if (MainScene.instance) MainScene.instance.exit()
			gen.current = undefined
		}
	}, [location.search, authorized])

	return <>
		<canvas id='canvas-main' ref={canvas} />
		<Ui
			stage={stage}
			location={location}
			isLoading={isLoading}
